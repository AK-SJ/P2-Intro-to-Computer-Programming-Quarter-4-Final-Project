...
/** RUN AND READ THE CONSOLE FOR DIRECTIONS **/
	...
	public void paint(Graphics g) { 
		...
		for(int i = 0; i < 6; i++) {
			...
			if(help == true) {
				...
				g.drawString(guess, 500, 80);
				
			}
			
			if(help2 == true) {
				
				g.drawString(guess, 500, 130);
				
			}

			if(help3 == true) {
	
				g.drawString(guess, 500, 180);
	
			}

			if(help4 == true) {
	
				g.drawString(guess, 500, 230);
	
			}

			if(help5 == true) {
	
				g.drawString(guess, 500, 280);
	...
	public void keyTyped(KeyEvent arg0) {
		...
		if(getLength() == 5) {
			...
			if(arg0.getKeyCode() == 10 /*Also if a word is inputted, so this could be an outer if-statement*/) {
					...
					if(guess == mysteryWord1) {
						
						help = true;
						
					}
					
					if(guess == mysteryWord2) {
						
						help2 = true;
						
					}

					if(guess == mysteryWord3) {
			
						help3 = true;
			
					}

					if(guess == mysteryWord4) {
			
						help4 = true;
			
					}

					if(guess == mysteryWord5) {
